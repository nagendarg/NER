...
class GLCN(nn.Module):
    ...
    def forward(self, x: Tensor, rel_features: Tensor, adj: Tensor, box_num: Tensor, box_labels: Tensor, **kwargs):
        ...
        # Concatenating the vis, text and pos features
        #x_1 = torch.cat((x[:,:,:512], pos), 2)

        '''
        tmp1 = torch.randn(2,10,256)
        tmp2 = torch.randn(2,10,256)
        tmp3 = torch.randn(2,10,256)

        #inputs = [torch.randn(2,10,100), torch.randn(2,10,120), torch.randn(2,10,20)]
        inputs = [tmp1[0,:,:], tmp2[0,:,:], tmp3[0,:,:]]
        out = mm(inputs) # torch.Size([10,300])
        print('======', out)
        '''

        # Feature fusion using BLOCK
        x_c = x.cpu()
        pos_c = pos.cpu()
        device = x.device
        #x_c = x
        ...
        for ii in range(x.shape[0]):
            ...
            inputs = [x_c[ii,:,:256], x_c[ii,:,256:512]]
            out = mm(inputs)

            inputs_1 = [out, pos_c[ii,:,:]]
            out_1 = mm(inputs_1)
            #Skipping positional features
            #out_1 = out

            out_1 = torch.unsqueeze(out_1, 0)

            if ii == 0:
                x_1 = out_1
                tmp = out_1
            else:
                x_1 = torch.cat((tmp, out_1), 0)
                tmp = x_1

        x_1 = x_1.to(device)

        '''
        inputs_1 = [x_c[0,:,:256], x_c[0,:,256:512]]
        inputs_2 = [x_c[1,:,:256], x_c[1,:,256:512]]
        #print(x[0,:,:256].is_cuda, x[0,:,256:512].is_cuda, pos[0,:,:].is_cuda)
        device = x.device
        #inputs_1 = inputs_1.cpu()
        #inputs_1 = torch(inputs_1)
        out_1 = mm(inputs_1)
        #print('out1', out_1.shape)
        #out_1 = torch.unsqueeze(out_1, 0)
        out_2 = mm(inputs_2)
        #out_2 = torch.unsqueeze(out_2, 0)

        pos_c = pos.cpu()
        inputs_3 = [out_1, pos_c[0,:,:]]
        inputs_4 = [out_2, pos_c[1,:,:]]

        out_3 = mm(inputs_3)
        out_3 = torch.unsqueeze(out_3, 0)
        out_4 = mm(inputs_4)
        out_4 = torch.unsqueeze(out_4, 0)

        x_1 = torch.cat((out_3, out_4), 0)
        x_1 = x_1.to(device)
        '''

        #print('-----------', x_1.shape, x_1.is_cuda)
        ...
        #Combining the GL feat with feature fusion features
        #x = x_1 + x_mod
