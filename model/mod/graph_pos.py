...
class GraphLearningLayer(nn.Module):
    def __init__(self, in_dim: int, learning_dim: int, gamma: float, eta: float):
        ...
        self.projection = nn.Linear(1027, learning_dim, bias=False)   #Change
...
class GCNLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.w_alpha = nn.Parameter(torch.empty(1027, out_dim))
        self.w_vi = nn.Parameter(torch.empty(1027, 1027))
        self.w_vj = nn.Parameter(torch.empty(1027, 1027))
        self.bias_h = nn.Parameter(torch.empty(1027))
        self.w_node = nn.Parameter(torch.empty(1027, out_dim))
...
class GLCN(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            if i==0:
                m = GCNLayer(in_dim_cur, 1027)
                in_dim_cur = 1027
                out_dim = in_dim_cur
            elif i==1:
                m = GCNLayer(in_dim_cur, 512)
                in_dim_cur = 512
                out_dim = in_dim_cur
            modules.append(m)
        self.gcn = nn.ModuleList(modules)

        self.alpha_transform = nn.Linear(6, 1027, bias=False)
    ...
    def forward(self, x: Tensor, rel_features: Tensor, adj: Tensor, box_num: Tensor, **kwargs):
        ...
        alpha = self.alpha_transform(rel_features)  #dim: [2, 40, 40, 512]
        #print(alpha.shape)
        ...
        for i, gcn_layer in enumerate(self.gcn):
            #print('i = ', i, 'x shape Before ', x.shape)
