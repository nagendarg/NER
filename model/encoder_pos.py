...
class Encoder(nn.Module):
    ...
    def forward(self, images: torch.Tensor, boxes_coordinate: torch.Tensor, transcripts: torch.Tensor,
        ...
        #height_feat[:,0] = abs(tmp[:,3] - tmp[:,1])/max(abs(tmp[:,3] - tmp[:,1]))
        ...
        transcripts_segments_new = transcripts_segments
        ...
        transcripts_segments_new = ranscripts_segments_new.reshape(B * N, T, D)
        ...
        trans_img = torch.cat((transcripts_segments_new, image_segments_new, loc_feat), 1)
