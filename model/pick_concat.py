...
from .encoder_concat import Encoder
from .graph_1024 import GLCN
...
class PICKModel(nn.Module):
    ...
    def compute_mask(mask: torch.Tensor):
        ...
        # (B * N, T)
        #print('graph', graph_node_mask.shape, graph_node_mask)

        # Mask for concatenated feat vector (B*N, 1024), mask shape (B*N, 1)
        mask_concat_vector = torch.logical_not(graph_node_mask.unsqueeze(-1).expand(B * N, 2)) & graph_node_mask.unsqueeze(-1).expand(B * N, 2)
        #print('maskk concat', mask_concat_vector.shape, mask_concat_vector)
        tmp_single_mask = torch.logical_not(graph_node_mask.unsqueeze(-1).expand(B * N, 1024)) & graph_node_mask.unsqueeze(-1).expand(B * N, 1024)
        tmp_single_mask = tmp_single_mask.transpose(0, 1).contiguous()

        graph_node_mask = graph_node_mask.unsqueeze(-1).expand(B * N, T)  # True for valid node
        
        #print('graph new', graph_node_mask)
        ...
        #print('shape', src_key_padding_mask.shape, src_key_padding_mask)
        return src_key_padding_mask, graph_node_mask, mask_concat_vector, tmp_single_mask
    ...
    def forward(self, **kwargs):
        ...
        src_key_padding_mask, graph_node_mask, mask_concat_vector, tmp_single_mask = self.compute_mask(mask)
        ...
        x, x_concat = self.encoder(images=whole_image, boxes_coordinate=boxes_coordinate, transcripts=text_emb,
                         src_key_padding_mask=src_key_padding_mask, text_mask=text_mask, mask_concat_vector=mask_concat_vector, tmp_single_mask=tmp_single_mask)
        ...
        # (B, N, D), (B, N, N), (B,)
        x_gcn, soft_adj, gl_loss = self.graph(x_gcn, relation_features[:,:,:,:5], init_adj, boxes_num)
