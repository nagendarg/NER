...
MAX_BOXES_NUM = 280  # limit max number boxes of every documents
...
class Document:
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
        ...
        try:
            ...
            if self.training:
                ...
                #print('File:', boxes_and_transcripts_file.as_posix())
                #print('boxes_and_transcripts_data', boxes_and_transcripts_data)
        ...
        boxes, transcripts, box_entity_types, box_label = [], [], [], []
        if self.training:
        ...
        box_label = [0 if i == 'Company_Name' or i == '"Company_Name"' else 1 if i =='Address' or i == '"Address"' else 2 if i =='Date' or i == '"Date"' else 3 if i =='Item_Name' or i == '"Item_Name"' else 4 if i=='Item_Price' or i == '"Item_Price"' else 5 if i=='Total' or i == '"Total"' else 6 if i=='Sub_Total' or i=='"Sub_Total"' else 7 if i=='Quantity' or i=='"Quantity"' else 8 if i=='Phone_Number' or '"Phone_Number"' else 9 if i=='Other' or i=='"Other"' else 10 if i=='Time' or i=='"Time"' else 11 if i=='Tax' or i=='"Tax"' else 12 if i=='Ind_Item_Price' or i=='"Ind_Item_Price"' else i for i in box_entity_types]
        
        #box_label = [0 if i == '"Company_Name"' else 1 if i =='"Address"' else 2 if i =='"Date"' else 3 if i =='"Item_Name"' else 4 if i=='"Item_Price"' else 5 if i=='"Total"' else 6 if i=='"other"' else i for i in box_entity_types]
        
        #print('box_label----', box_label)

        '''
        box_label[box_label == 'Company_Name'] = 0
        box_label[box_label == '"Company_Name"'] = 0
        box_label[box_label == 'Address'] = 1
        box_label[box_label == '"Address"'] = 1
        box_label[box_label == 'Date'] = 2
        box_label[box_label == '"Date"'] = 2
        box_label[box_label == 'Item_Name'] = 3
        box_label[box_label == '"Item_Name"'] = 3
        box_label[box_label == 'Item_Price'] = 4
        box_label[box_label == '"Item_Price"'] = 4
        box_label[box_label == 'Total'] = 5
        box_label[box_label == '"Total"'] = 5
        box_label[box_label == 'other'] = 6
        box_label[box_label == '"other"'] = 6
        '''
        

        #print('text', transcript)
        #print('box_entity_types', box_entity_types, '\n label', box_label)

        box_cord = torch.FloatTensor(boxes)
        ...
        relation_features = np.zeros((boxes_num, boxes_num, 7))
        ...
        try:
            ...
            #print('Box_num: ', boxes_num, 'Resized Length: ', len(resized_boxes))
            #print(box_cord.shape, torch.FloatTensor(resized_boxes).shape)
            ...
            texts = texts[:, :transcript_len].to("cpu").numpy()
            #texts = texts[:, :transcript_len].numpy()
            texts_len = np.clip(texts_len.to("cpu").numpy(), 0, transcript_len)
            ...
            self.boxes_coordinate_org = box_cord                                #Added by Nag
            self.boxes_label = box_label
    ...
    def relation_features_between_ij_nodes(self, boxes_num, i, min_area_boxes, relation_features, transcript_i,
        ...
        for j in range(boxes_num):
            ...
            tj = 0
            for char in transcript_j:
                if char.isalpha():
                    tj += 1
                    break

            if ti == 0 and tj == 0:
                relation_features[i, j, 5] = 1 \
                    if height_j * (height_i) is not None else -1  # T_j/T_i
                relation_features[i, j, 6] = 1 \
                    if height_j * (height_i) is not None else -1  # T_j/T_i

            elif ti != 0 and tj != 0:
                relation_features[i, j, 5] = 0 \
                    if height_j * (height_i) is not None else -1  # T_j/T_i
                relation_features[i, j, 6] = 0 \
                    if height_j * (height_i) is not None else -1  # T_j/T_i
