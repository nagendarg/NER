...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.op_droput = nn.Dropout(self.dropout)
    ...
    def forward(self, images: torch.Tensor, boxes_coordinate: torch.Tensor, boxes_coordinate_org: torch.Tensor, transcripts: torch.Tensor,
        ...
        angle = torch.zeros(B, N, N, 1, device=images.device)
        unit_vector = torch.zeros(B, N, 2)
        ...
        #print(top, bot)
        for i in range(B):
            ty = torch.flatten(tmp[i,:,1])
            tx = torch.flatten(tmp[i,:,0])
            t = ty > 0
            top[i] = min(ty[t])
            bot[i] = max(tmp[i,:,3])
            ...
            '''
            pos_feat[i,:,0] = (tmp[i,:,0] - left[i])/(right[i]-left[i])
            pos_feat[i,:,1] = (tmp[i,:,1] - top[i])/(bot[i]-top[i])
            pos_feat[i,:,2] = (tmp[i,:,2] - left[i])/(right[i]-left[i])
            pos_feat[i,:,3] = (tmp[i,:,3] - top[i])/(bot[i]-top[i])
            '''
            ...
            # Computing the angle between the nodes
            # Unit vector
            unit_vector[i,:,0] = pos_feat[i,:,0] 
            unit_vector[i,:,1] = pos_feat[i,:,1]
            
            vec_norm = np.linalg.norm(unit_vector[i,:,:].to("cpu").numpy(), axis=1)
            vec_norm = torch.tensor(vec_norm)
            vec_norm = vec_norm.unsqueeze(1)
            #print(unit_vector.shape, vec_norm.shape)

            unit_vector[i,:,:] = torch.div(unit_vector[i,:,:], vec_norm)
            
            unit_vector[torch.isnan(unit_vector)] = 0
            

            for a1 in range(0, N):
                for a2 in range(0, N):
                    dot_product = torch.dot(unit_vector[i,a1,:], unit_vector[i,a2,:])
                    angle[i, a1, a2, 0] = torch.acos(dot_product)
            angle[torch.isnan(angle)] = -1
        #print(unit_vector, angle[0,0,:,:])    
        #print(angle[0,0,:,:])
        #print(pos_feat)
        ...
        ######################
        #print('New Transcript Segments Shape: ', transcripts_segments_new.shape, 'New Image Segment Shape: ', image_segments_new.shape, 'Pos_feat: ', pos_feat.shape)
        #New Transcript Segments Shape:  torch.Size([140, 512]) New Image Segment Shape:  torch.Size([140, 512])

        # Normalizing text and visual features independently - Layer Norm
        #transcripts_segments_new = self.norm_256(transcripts_segments_new)
        #image_segments_new = self.norm_256(image_segments_new)
        
        # position features - projecting into 256 dim
        pos_emb = self.pos_transform(torch.cat((pos_feat, height_feat),1))

        #trans_img = torch.cat((transcripts_segments_new, image_segments_new, pos_feat, height_feat), 1)
        trans_img = torch.cat((transcripts_segments_new, image_segments_new, pos_emb), 1)
        ...
        #print('=============', image_segments.shape, transcripts_segments.shape, pos_emb.shape)
        # (B*N, 1, D)
        ...
        pos_emb = pos_emb.expand_as(transcripts_segments)

        #print('Image Segment Shape: ', image_segments.shape, transcripts_segments.shape)
        ...
        out = image_segments + transcripts_segments + pos_emb
        #out = transcripts_segments
        ...
        out = self.op_droput(out)
        #print('----------', out.shape, trans_img.shape, image_segments.shape, transcripts_segments.shape)

        return out, trans_img, image_segments, angle
        #return out
