...
import model.main_model as arch_module
from data_utils import main_dataset as dataset_module

from data_utils.main_dataset import BatchCollateFn
...
def main(config: ConfigParser, local_master: bool, logger=None):
    ...
    main_model = config.init_obj('model_arch', arch_module)
    logger.info(f'Model created, trainable parameters: {main_model.model_parameters()}.') if local_master else None

    # build optimizer, learning rate scheduler.
    optimizer = config.init_obj('optimizer', torch.optim, main_model.parameters())
