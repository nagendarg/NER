...
class BiLSTMLayer(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor,
        ...
        '''
        #tmp = torch.as_tensor(sorted_lengths, dtype=torch.int64, device='cpu')
        #tmp = sorted_lengths.to(torch.int64)
        tmp = torch.as_tensor(sorted_lengths, dtype=torch.int64, device='cpu') 
        tmp = tmp.cpu()
        tt = torch.zeros(len(tmp), dtype=torch.int64).cpu()

        #ll = torch.tensor([int(tmp[0])], device='cuda')
        #ll = torch.tensor(ll, dtype=torch.int64).cpu()
        print(int(tmp[0]), tmp.shape, tmp)

        print('*************', type(sorted_lengths), sorted_lengths.type(), x_seq.type(), tmp.type(), x_seq.shape, '...', tmp.device, x_seq.device)
        #packed_x = nn.utils.rnn.pack_padded_sequence(x_seq, lengths=tmp.to('cpu'), batch_first=True)
        '''
