...
class BatchCollateFn(object):
    ...
    def __call__(self, batch_list: List[Document]):
        ...
        #print('1111111111**********************', boxes_coordinate_batch_tensor.shape)

        #print('2222222222', boxes_coordinate_batch_tensor)
        #for i, x in enumerate(batch_list):
            #print(i)
            #print('num', x.boxes_num, 'diff', max_boxes_num_batch - x.boxes_num)
            
            #print('2222222', x.boxes_coordinate_org.shape)
        #print('...............................')

        #Original box coordinates
        boxes_coordinate_padded_list_1 = [F.pad(torch.FloatTensor(x.boxes_coordinate_org),
                                              (0, 0, 0, max_boxes_num_batch - x.boxes_num))
                                        for i, x in enumerate(batch_list)]
        
        #print('-------', np.array(boxes_coordinate_padded_list_1).shape)
        boxes_coordinate_batch_tensor_1 = torch.stack(boxes_coordinate_padded_list_1, dim=0)

        #print('boxes_coordinate_batch_tensor_1', boxes_coordinate_batch_tensor_1)
        
        '''
        for i, x in enumerate(batch_list):
            print('i-----------------', i)
            tmp = torch.FloatTensor(x.boxes_label)
            #tmp_1 = torch.FloatTensor(x.boxes_coordinate_org)
            print('shape', tmp.shape)
            #print(max_boxes_num_batch, x.boxes_num)
            #print()
            #print('shapes', x.boxes_coordinate_org.shape, x.boxes_label.shape)
        '''
        
        #Box label
        '''
        #box_label = torch.zeros(2,)
        for i, x in enumerate(batch_list):
            #box_label = torch.stack()
            print(x.boxes_label)
            a = x.boxes_label
            print('x box', len(x.boxes_label), max_boxes_num_batch, x.boxes_num)
        '''

        boxes_label_padded = [F.pad(torch.IntTensor(x.boxes_label), (0, max_boxes_num_batch - x.boxes_num)) for i, x in enumerate(batch_list)]
        
        
        #print('pad', boxes_label_padded.shape, x.boxes_num)
        
        
        box_label = torch.stack(boxes_label_padded, dim=0)
        #print(boxes_label_padded.shape , box_label.shape)
        ...
        if self.training:
            batch = dict(whole_image=image_batch_tensor,
                         ...
                         boxes_coordinate_org = boxes_coordinate_batch_tensor_1,
                         boxes_label = box_label,
