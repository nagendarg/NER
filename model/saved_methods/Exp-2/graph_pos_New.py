...
class GraphLearningLayer(nn.Module):
    def __init__(self, in_dim: int, learning_dim: int, gamma: float, eta: float, dropout: float = 0.1,):
        ...
        self.projection_post = nn.Linear(3, 128, bias=False)
        self.projection_tot = nn.Linear(3*128, 128, bias=False)
    ...
    def forward(self, x: Tensor, adj: Tensor, box_num: Tensor = None):
        ...
        x_hat = self.projection_tot(x_hat)
...
class GCNLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.w_vi = nn.Parameter(torch.empty(128, 256))
        self.w_vj = nn.Parameter(torch.empty(128, 256))
...
class GCNLayer_layer_1(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.w_vi = nn.Parameter(torch.empty(256, 256))
        self.w_vj = nn.Parameter(torch.empty(256, 256))
        self.bias_h = nn.Parameter(torch.empty(256))
        self.w_node = nn.Parameter(torch.empty(256, out_dim))
...
class GLCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.alpha_transform = nn.Linear(5, 256, bias=False)
