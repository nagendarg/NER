...
class GraphLearningLayer(nn.Module):
    def __init__(self, in_dim: int, learning_dim: int, gamma: float, eta: float):
        ...
        self.projection = nn.Linear(1024, learning_dim, bias=False)   #Change
...
class GCNLayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.w_alpha = nn.Parameter(torch.empty(1024, out_dim))
        self.w_vi = nn.Parameter(torch.empty(1024, 1024))
        self.w_vj = nn.Parameter(torch.empty(1024, 1024))
        self.bias_h = nn.Parameter(torch.empty(1024))
        self.w_node = nn.Parameter(torch.empty(1024, out_dim))
...
class GLCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.alpha_transform = nn.Linear(6, 1024, bias=False)
