...
from .encoder_pos_New import Encoder
#from .graph_pos_New import GLCN
from .graph_block import GLCN
...
class PICKModel(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        box_labels = kwargs['boxes_label']
        #print('label', box_labels.shape, box_labels)

        #print(text_segments[0,0,:])
        ...
        #print('pick, box, text_seg, mask, whole_image', boxes_coordinate.shape, text_segments.shape, mask.shape, whole_image.shape)
        x, x_concat, image_segments, angle = self.encoder(images=whole_image, boxes_coordinate=boxes_coordinate, boxes_coordinate_org=boxes_coordinate_org, transcripts=text_emb,
                         src_key_padding_mask=src_key_padding_mask, text_mask=text_mask)

        #x is [B*N, T, D]
        #print('After Encoder', x.shape, x_concat.shape)
        ...
        # (B, N, D), (B, N, N), (B,)
        x_gcn, soft_adj, gl_loss = self.graph(x_gcn, torch.cat((relation_features, angle), 3) , init_adj, boxes_num, box_labels)
        ...
        #print('before decoder', x.shape, x_gcn.shape, x.reshape(B, N, T, -1).shape, src_key_padding_mask.shape, text_length.shape)
        ...
        #logits, new_mask, log_likelihood = self.decoder(image_segments, src_key_padding_mask, x, x.reshape(B, N, T, -1), x_gcn, mask, text_length,
        #                                                iob_tags_label)                                            
