...
class UnionLayer(nn.Module):
    ...
    def forward(self, I: Tensor, src_key_padding_mask: torch.Tensor, x: Tensor, x_1:Tensor, x_gcn: Tensor, mask: Tensor, length: Tensor, tags):
        '''
        ...
        #x = x.reshape(B, N * T, -1)
        #I = I.reshape(B, N * T, -1)
        #src_key_padding_mask = src_key_padding_mask.reshape(B, N * T)
        mask = mask.reshape(B, N * T)

        #print('x_gcn', x_gcn.shape)
        x_gcn = x_gcn.reshape(B*N, -1)
        x_gcn = x_gcn.unsqueeze(1).expand(B*N, T, -1)
        ...
        # (B, N*T, D)
        #print('shape', x.shape, x_gcn.shape, I.shape)
        x = x_gcn + x + I
...
class Decoder(nn.Module):
    ...
    def forward(self, I: Tensor, src_key_padding_mask: torch.Tensor, x: Tensor, x_1: Tensor, x_gcn: Tensor, mask: Tensor, length: Tensor, tags: Tensor):
        '''
        ...
        new_x, new_mask, doc_seq_len, new_tag = self.union_layer(I, src_key_padding_mask, x, x_1, x_gcn, mask, length, tags)
